...
String nowUrl="&back=special_06&focus_id="+focus_id+"&programa_id="+programa_id+"&perv_num="+perv_num+"&perv_count="+(perv_count+1)+"&pf_focus_id="+pf_focus_id+"&home_focus_id="+home_focus_id+"&home_from="+home_from+"&backUrl="+backUrl_encode;
...
<head>
	...
	<script type="text/javascript" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/js/public_animation.js"></script>
	...
	<script type="text/javascript">
		...
		function F_KEY_G_0() {
			if(from==''){
				...
				}else if('animation_index'==back){
					window.location.href = "animation_index.jsp?programType=<%=programType+pervUrl%>"; //区分列表和首页
				}else if('animation_special_list'==back){
					window.location.href = "animation_special_list.jsp?programType=<%=programType+pervUrl%>"; //jsp修改返回列表参数	
				}
        	}else{
		...
		function F_MORE_BACK(){
			window.location.href = "animation_special_list.jsp"; //jsp修改返回列表参数	
		}
		...
		var buttons = [

			{
				...
				upButtonId: 'bt_back',
				downButtonId: '',
				leftButtonId: 'bt_zf-1',
				rightButtonId: 'bt_zf1',
				blurFun: '',
				focusFun: 'F_FOCUS',
				enterFun: 'F_E_F0',
				focusImage: 'anime-special-f.png',
				blurImage: 'Pixel.png'
			},
			...
			},
			{
				...
				blurFun: '',
				focusFun: '',
				enterFun: 'F_MORE_BACK',
				focusImage: 'anime-special-back-.png',
				blurImage: 'Pixel.png'
			},
			{
				id: 'bt_pdg_f',
		...
		for(int i=0;i<list.size();i++){
	    	...
	    	_stat+="-"+topicSeries.getTitle();
		%>
		function F_E_F<%=i%>() {
			var _cpcode = '<%=topicSeries.getCpCode()%>';
            var _code = '<%=topicSeries.getCode()%>';
            var _id = '<%=topicSeries.getId()%>';
            ...
            commen_play(_id,_programType,_programType2,_code,_cpcode,'<%=nowUrl%>');
		}
		<%}%>
...
<body onload="init()" bgcolor="transparent">
	<%
		String bigpic="";
		if(list.size()>0){
			bigpic=list.get(0).getPosterUrl2();
		}
	%>
	<!-- jsp修改，背景大海报 -->
	<img src="<%=bigpic %>" style="position:absolute;left:0px;top:0px;width:1280px;height:720px;">
	

	<div id="DIV_F" style="position: absolute; left: 6px; top: 344px; z-index: 11;">
		<img id="IMG_F" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/Pixel.png">
	</div>
	<!-- jsp修改，循环html  src=""底部小海报  -->
	...
	<%
   	...
   	%>
	<div style="position:absolute;left:<%=left%>px;top:406px;">
		<img id="bt_img<%=i %>" src="<%=topicSeries.getPosterUrl1() %>" style="position: absolute;left:0px;top:0px;width:181px;height:268px;;z-index:1;">
	</div>
	<div style="position:absolute;left:<%=i*198+6%>px;top:344px;">
		<img id="bt_zf<%=i %>" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/Pixel.png" style="position: absolute;left: 0px; top:0px;width:263px;height:372px;;z-index:10;">
	</div>
	<%} %>

	<div style="position:absolute;left:1150px;top:6px;">
		<img src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/anime-back01.png" style="position: absolute; left: 0px; top:0px;;z-index:1;">
	</div>
	<div style="position:absolute;left:1150px;top:6px;">
		<img id="bt_back" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/Pixel.png" style="position: absolute; left: 20px; top:5px;width:87px;height:78px; ;z-index:99;">
	</div>
