...
<%
    ...
    String pervUrl="&back="+back+"&cartoon_code="+cartoon_code+"&focus_id="+focus_id+"&programa_id="+programa_id+"&perv_num="+perv_num+"&perv_count="+(perv_count+1)+"&pf_focus_id="+pf_focus_id+"&home_focus_id="+home_focus_id+"&home_from="+home_from+"&home_perv_num="+home_perv_num+"&sanfocus_id="+sanfocus_id+"&sanperv_num="+sanperv_num;
      ...
      // var returnUrl= "http://117.71.39.6/tyyq-test/detail.jsp?id=<%=id%>&programType=<%=programType%>&code=<%=code%>&perv_count=0&back=list&cpcode=<%=cpcode%><%=pervUrl%>";
        ...
        function backPrograma2() {
            F_M_10();
             ...
             }else if(back.indexOf("special_06")!=-1){
            	 window.history.go(-1)
             }else if ("anime" == "<%=programType%>") {
                window.location.href = "list.jsp?programType=<%=programType+pervUrl%>";
            } else {
                window.history.go(-1)
            }
        }
        ...
        function updateWatchRecord() { //提交当前播放时间
            ...
            if ((watch_time > 10 && currentPlayTime <= 10) || (currentPlayTime && currentPlayTime > 10) || <%=iscut%> ==1 || tvflag) {
            	var stbModel = Authentication.CTCGetConfig("stbModel");
            	var url = webset.api + "usr/demand.json?userid=" + userId + "&stb_model="+stbModel+"&videotype=video&videoid=" + videoid +"&cuttime=" + parseInt(currentPlayTime) + "&series=<%=code%>&cpcode=<%=cpcode%>&title=" + title +"&dramacount=" + cutdrama + "&opttype=1&from=bofangqi&play_time=" + (playtime / 1000);
...
<body bgcolor="transparent"  onload="init_mp();" onunload="F_M_10()">
    ...
    <div id="order_tips" style="left: 0px; top: 563px; width: 1280px; display: none; position: absolute;">
        <a id="bt_ordertips" style="left: 6px; top: 8px; position: absolute;" href="javascript:order_soon()">
         <img width="1" height="1" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/Pixel.png" border="0"> </a>
