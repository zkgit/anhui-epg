...
<head>
    ...
    <script type="text/javascript">
        ...
        var cutdrama='<%=programType%>'=='film'?1:<%=cutdrama%>;
        var dramasum=<%=dramasum%>;
        var title='<%=title%>';
        var videoid="<%=videoid%>";
        var playUrl="<%=playurl%>";
        var watch_time =<%=cutTime%>?<%=cutTime%>/1000:0;
        var cuttime=getCookie('<%=programType%>time')?JSON.parse(getCookie('<%=programType%>time')):{};
        if('<%=programType%>'!='film'){
            var urlarr=[{cutdrama:{playurl:"<%=playurl%>",code:"<%=videoid%>"}}];
              getDrama()
         }
        var focus_id = 'bt_f0';
        ...
        function watchRecord() { //不太清楚干嘛的
            ...
            if (watch_time > 10) {
                _SELECT('watchRecord').src = "http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/record.png?action=updateWatchRecord&record=yes&position=1&t=" +
                    t + "&watch_time=" + watch_time;
            }
        }
        ...
        if(currentPlayTime){currentPlayTime=parseInt(currentPlayTime)}
        if(watch_time>10&&currentPlayTime<=10){currentPlayTime=0;}
       if((watch_time>10&&currentPlayTime==0)||(currentPlayTime&&currentPlayTime>10)||<%=iscut%>==1||tvflag){
          var url = webset.api+"usr/demand.json?userid="+userId+"&videotype=video&videoid="+videoid+"&cuttime="+(currentPlayTime)*1000+"&series=<%=code%>&cpcode=<%=cpcode%>&title="+title+"&dramacount="+cutdrama+"&opttype=1&play_time="+(playtime/1000);
            if(currentPlayTime<=10){currentPlayTime=1;}
            if('<%=programType%>'=='film'){
                cuttime[<%=id%>]=currentPlayTime*1000;
             }else if('<%=programType%>'=='tv'){
               cuttime[<%=id%>]={time:currentPlayTime*1000,num:cutdrama,code:videoid};
            }
          setCookie('<%=programType%>time', JSON.stringify(cuttime))
        ...
        function F_KEY_G_2() {
            //	window.location.href='/zyjh/play.jsp?action=playItemByPosition&whichMenu=one&programa_id=9a69b3deb76c4d7199e25731eee072f0&item_id=5204284&position=-1';
        }
        ...
        function F_PLAY_NEXT() {
            //            window.location.href='/zyjh/play.jsp?action=playItemByPosition2&whichMenu=one&programa_id=9a69b3deb76c4d7199e25731eee072f0&item_id=5204284&position=2&seriesCp_id=5204284&item_cp=voole&playNext=playNext';
        }
            ...
            if (show_vs == 'yes') { //剧集在显示
                ...
                if (play_states == "play") {
                   ...
                   playtime=(new Date().getTime()-start)+playtime;
                    ...
                    //		_SELECT("div_vs").style.display = "block";_H_F(_BS[focus_id]['upButtonId']);
                    // 	v_s='yes';show_vs='yes';focus_id='play_button';
                    // 	page_now=parseInt((video_episode_now/13))+1;
                    // 		F_KEY_G_7();
        ...
        function F_KEY_G_6() { //右
            var bt_w = focus_id.replace('f', 'w');
            var htm = _SELECT(bt_w).innerHTML.replace(/\D/g, "");
            if (show_vs == 'yes' && htm == '') { //更多
                if (((page_now - 1) * 13 + (13)) < position_sum) {
                    page_now++;
                    show_vs2 = 'yes2';
                    focus_id = 'bt_f0';
                    F_KEY_G_7();
        ...
        var position_sum ='<%=programType%>'=='film'?1:dramasum;
        var page_now = 1;
        var show_vs = 'no';
        var show_vs2 = 'no';
        var index_now = 0;
    ...
    function getDrama(){
        $.get(webset.api + 'video/drama.json?userid='+userId+'&code=<%=code%>&cpCode=<%=cpcode%>', function (res) {
             var res = eval("(" + res + ")");
                if(res.response&&res.response.responseHeader.code==200){
                    ...
                    if(!dramasum){
                       dramasum=position_sum=parseInt(res.response.responseBody.dramaCount);
                       var jiance= setInterval(jiancet(), 10000);
                    }
                }
        }, false);
    }
        function F_KEY_G_7() { //下 显示剧集
            ...
            if (show_vs2 == 'yes') {
                ...
                if(show_vs2 != 'yes2'){
                    page_now = parseInt((cutdrama-1)/ 13) + 1;
                }
                var html ="<div style='position:absolute;left:0px;top:645px;' ><img src='http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-bg.png' width='1280px' height='65px'></div>";
                var _i = 0;
                for (var i = 0; i < 13 && ((page_now - 1) * 13 + (i + 1)) <= position_sum; i++){
                    if ((cutdrama-1) == ((page_now - 1) * 13 + i)) {
                        var bt_w = "<div id='bt_w" + i + "' style='position:absolute; left:" + (20 + i * 88) +
                            "px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20;'>" +
                            ((page_now - 1) * 13 + (i + 1)) + "</div>";
                        index_now = i;
                    } else {
                        var bt_w = "<div id='bt_w" + i + "' style='position:absolute; left:" + (20 + i * 88) +
                            "px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20;'>" +
                    ...
                    var bt_f = "<img id='bt_f" + i + "' src='http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png' style='position:absolute;left:" + (20 + i *
                        88) + "px;top:666px;'/>";
                    html = html + bt_w + bt_f;
                    _i = i;
                }
                _i++;
                if (((page_now - 1) * 13 + (13)) < position_sum) {
                    var bt_w = "<div id='bt_w" + _i + "' style='position:absolute; left:" + (20 + _i * 88) +
                        "px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20;'>>></div>";
                    var bt_f = "<img id='bt_f" + _i + "' src='http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png' style='position:absolute;left:" + (20 + _i *
                        88) + "px;top:666px;'/>";
                    html = html + bt_w + bt_f;
                } else if (page_now > 1){
                    _i--;
        ...
        function resetdata(drama,url,id){
         ...
         _SELECT('cuttitle').innerHTML="当前播放："+title;
          ...
          watch_time=0;
          var mediaa = '[{mediaUrl:"' + playUrl +'",mediaCode: "vod1",mediaType:2,audioType:1,videoType:3,streamType:2,drmType:1,fingerPrint:1,copyProtection:0,allowTrickmode:1,startTime:0,endTime:0,entryID:"entry1"}]';
        ...
        function playdrama(i){
            ...
            if(urlarr[i]){
                ...
                urlarr.push({i:{playurl:res.response.responseBody[0].url,code:res.response.responseBody[0].code}})
                }
                },false);
            }
        ...
        function F_F_E(){ //剧集enter
            var bt_w = focus_id.replace('f', 'w');
            var i = parseInt(_SELECT(bt_w).innerHTML.replace(/\D/g, ""));
            if (i == ''){ //更多
                if (((page_now - 1) * 13 + (13)) < position_sum) {
                    page_now++;
                    focus_id = 'bt_f0';
        ...
        function F_YY_PLAY(arg) {
            var htm = arg; //video_episode[arg-1];
            window.location.href =
                '/zyjh/play.jsp?action=playItemByPosition2&whichMenu=one&programa_id=9a69b3deb76c4d7199e25731eee072f0&item_id=5204284&position=' +
                htm + '&seriesCp_id=5204284&item_cp=voole&playNext=playNext';
        }
...
<body  onload="init_mp();" onunload="F_M_10()">
    ...
    <div id="videoNameTips" style="left: 0px;width: 1280px;height: 50px;text-align: center;margin-top: 0px;position: absolute;opacity: 0.8;background-color: rgb(0, 0, 0);/* display: none; */">
        <font style="text-align: center; color: rgb(255, 255, 255); font-size: 32px;" id="cuttitle">当前播放：<%=title%></font>
    </div>
    ...
    <img id="logo" style="left: 1110px; top: 2px; position: absolute;" src="http://apkpic.kanketv.com.ahct.lv1.vcache.cn/image/CPPhoto/<%=cpcode%>.png">
    <!--<div id="process_bar_id" style="position:absolute;left:0px;top:0px;width:1280px;height:720px; display:none;" >
<div id="pb_bg_id" style="position:absolute;left:0px;top:600px;width:1280px;height:120px;background-color:#000000;opacity: 0.8;" ></div>
<div id="end_time_id" style="position:absolute;left:1160px;top:652px;width:100px;height:60px;color:#FFF;text-align:center;font-size: 21px;">/00:00:00</div>
<div id="duration_bg_id" style="position:absolute;left:80px;top:660px;width:990px;height:10px;background-color:grey;"></div>
<div id="played_id" style="position:absolute;left:80px;top:660px;width:10px;height:10px;background-color:#ff9933;"></div>
<div id="show_time_id" style="position:absolute;left:1068px;top:652px;width:100px;height:60px;color:#FFF;text-align:center;font-size:21px;">00:00:00</div>
<div id="play_icon_id" style="position:absolute;left:28px;top:644px;width:50px;height:46px;">
<img id="play_button" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/volumn_icon/play-play.png" style="width: 50px;height:46px;"/>
</div>
</div>-->
    ...
    <div id="div_vs" style="left: 0px; top: 0px; position: absolute;display: none;">
        <div style="position:absolute;left:0px;top:655px;">
            <img src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-bg.png" width="1280px" height="65px">
        </div>
        <div id="bt_w0" style="position:absolute; left:20px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20; display: none1;">1</div>
        <div id="bt_w1" style="position:absolute; left:108px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20; display: none1;">2</div>
        <div id="bt_w2" style="position:absolute; left:196px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20; display: none1;">3</div>
        <div id="bt_w3" style="position:absolute; left:284px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20; display: none1;">4</div>
        <div id="bt_w4" style="position:absolute; left:372px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20; display: none1;">5</div>
        <div id="bt_w5" style="position:absolute; left:460px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20; display: none1;">6</div>
        <div id="bt_w6" style="position:absolute; left:548px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20; display: none1;">7</div>
        <div id="bt_w7" style="position:absolute; left:636px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20; display: none1;">8</div>
        <div id="bt_w8" style="position:absolute; left:724px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20; display: none1;">9</div>
        <div id="bt_w9" style="position:absolute; left:812px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20; display: none1;">10</div>
        <div id="bt_w10" style="position:absolute; left:900px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20; display: none1;">11</div>
        <div id="bt_w11" style="position:absolute; left:988px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20; display: none1;">12</div>
        <div id="bt_w12" style="position:absolute; left:1076px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20; display: none1;">13</div>
        <div id="bt_w13" style="position:absolute; left:1164px;top:668px;width:80px; height:40px; line-height:40px; text-align:center; font-size:28px;font-family:微软雅黑; font-weight:normal; color:#ffffff; overflow:hidden; margin:0; padding:0;z-index: 20; display: none1;">>></div>
        <img id="bt_f0" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png" style="position:absolute;left:20px;top:666px;  display: none1;" />
        <img id="bt_f1" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png" style="position:absolute;left:108px;top:666px; display: none1;" />
        <img id="bt_f2" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png" style="position:absolute;left:196px;top:666px; display: none1;" />
        <img id="bt_f3" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png" style="position:absolute;left:284px;top:666px; display: none1;" />
        <img id="bt_f4" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png" style="position:absolute;left:372px;top:666px; display: none1;" />
        <img id="bt_f5" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png" style="position:absolute;left:460px;top:666px; display: none1;" />
        <img id="bt_f6" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png" style="position:absolute;left:548px;top:666px; display: none1;" />
        <img id="bt_f7" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png" style="position:absolute;left:636px;top:666px; display: none1;" />
        <img id="bt_f8" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png" style="position:absolute;left:724px;top:666px; display: none1;" />
        <img id="bt_f9" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png" style="position:absolute;left:812px;top:666px; display: none1;" />
        <img id="bt_f10" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png" style="position:absolute;left:900px;top:666px; display: none1;" />
        <img id="bt_f11" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png" style="position:absolute;left:988px;top:666px; display: none1;" />
        <img id="bt_f12" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png" style="position:absolute;left:1076px;top:666px; display: none1;" />
        <img id="bt_f13" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-dra-b.png" style="position:absolute;left:1164px;top:666px; display: none1;" />
    </div>
    ...
    <div id="order_tips" style="left: 0px; top: 563px; width: 1280px; display: none; position: absolute;">
        <a id="bt_ordertips" style="left: 6px; top: 8px; position: absolute;" onfocus="MM_swapImage('bt_ordertips_f','','http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/window-order-bg_f.png',1);"
            onblur="MM_swapImgRestore();" href="javascript:order_soon()">
            <img width="1" height="1" src="http://epgpic.kanketv.com.ahct.lv1.vcache.cn/itv-epg/images/Pixel.png" border="0"> </a>
        ...
        <div id="dd" style="display:none;left: 0px; top: 400px; position: absolute;width:600px;height:50px; word-wrap: break-word;background:#fff;color:red;font-size:16px;line-height:20px;"></div>
    <script type="text/javascript">
      //  setTimeout('watchRecord()', 10000);
        ...
        var total_times = mp.getMediaDuration();
        if((total_times-currentPlayTime<=30)&&!flag&&total_times>0){
        flag=true;
        ...
        _SELECT("palyNextTips").style.display = "block";
        setTimeout('playdrama(cutdrama+1)',3000);
        }
        }
        }
        if(dramasum||'<%=programType%>'=='film'){
          var jiance= setInterval(jiancet(), 10000);
        }
                ...
                setInterval(function () {
                    if (show_times > tryWatch || mp.getCurrentPlayTime() > tryWatch) {
                        if ('dy' == 'dy') {
                            ...
                            itemname = encodeURIComponent(encodeURIComponent(itemname));
                            window.location.href =
                                "/zyjh/order/userOrder.jsp?item_cp=voole&item_id=4136&item_name=" + itemname;
                        } else {
